# -*- coding: utf-8 -*-
"""
Created on Thu Nov 12 03:00:56 2020
...
class experimento():
  ...
  def solucion(self):
    ...
    omega=math.sqrt(-m)
    a=max(self.ordenada)
    a1=np.linspace(0,10,1000)
    theta=a*np.cos(omega*a1)+l
    return (a,omega,erroromega,l,errorl,a1,theta) 
  def visualizar(self):   
    k=experimento(self.abscisa,self.ordenada)
    a,omega,erroromega,l,errorl,a1,theta=k.solucion()
    return (print("La función es: X=",a,"cos( (",omega,"+/-",erroromega,")t) + (",l,"+/-",errorl,")"),plt.plot(a1,a*np.cos(omega*a1)+l))
  def posicionx(self):
